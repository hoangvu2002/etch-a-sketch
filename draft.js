...
function createGrid(rows, columns) {
  ...
  for (let i = 0; i < rows*columns; i++) {
    ...
    rowDiv.addEventListener('mouseenter', changeColor );
...
//
  ...
  function changeColor() {
    if (this.style.backgroundColor === 'gray') { // Check if color is already set
      this.style.backgroundColor = getRandomColor(); // Apply new random color
    }
  }
...
resetButton.addEventListener('click', function() {
  ...
  cellDivs.forEach(function(cellDiv) {
    if (cellDiv.style.backgroundColor !== 'gray') { // Check if color is not gray
